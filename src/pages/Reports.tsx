  ...
  return (
    <Shell title="Reports">
      <div className="eduwatt-print-hide" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        <Kpi label="Solar This Month" value={monthSolar} unit="kWh" />
        <Kpi label="Grid This Month" value={monthGrid} unit="kWh" />
        <Kpi label="CO₂ Avoided" value={monthCo2} unit="kg" />
        <Kpi label="Cost Saved" value={monthSavedUzs} unit="UZS" />
      </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text-meta)", marginTop: 4 }}>
              {mockData.schoolName} · {new Date().toLocaleDateString(undefined, { month: "long", year: "numeric" })}
            </div>
          </div>
          <button
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ color: "var(--text-meta)", textAlign: "left" }}>
                ...
                <th style={{ padding: "6px 8px" }}>Solar (kWh)</th>
                <th style={{ padding: "6px 8px" }}>Grid (kWh)</th>
                <th style={{ padding: "6px 8px" }}>CO₂ (kg)</th>
              </tr>
            </thead>
            <tbody>
              {week.map((d: any) => (
